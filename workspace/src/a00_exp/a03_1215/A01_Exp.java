...
public class A01_Exp {
	public static void main(String[] args) {
		...
		// private : 상속에 의해서 하위 클래스가 직접적인 접근을 하지 못 한다.
		// X(default) : 하위 클래스가 패키지가 다를 때, 접근을 하지 못 한다.
		// protected, public : 외부 패키지일지라도 접근이 가능.
		// 필드를 접근 하지 위하여 public으로 된 setXXX(), getXXX()메서드로 간접접근을 한다.
		...
		// 자식클래스 입장에서 여러 부모 클래스를 상속하지 못 한다는 말이다..
		...
		// 1. default 생성자//매개변수 없는 생성자는 
		// super()를 기본으로 포함하기때문에 하위에서 특별한 선언없이 사용할 수 있다.
		// 2. 상위에 매개변수 있는 생성자를 선언시, 하위 생성자에서는 해당 생성자의 type, 갯수에
		// 맞게 생성자 선언 첫번째라인에 호출하여야 한다.
		...
		// overloding : 같은 클래스 내에서 생성자나 메서드가 이름이 같더라도, 매개변수의
		// 개수, Type, 순서로 선언이 가능한 것을 말한다.
		// overriding : 상속관계에 있는 하위객체가 메서드를 상위의 객체의 메서드와 동일한
		// 이름과 매개변수로 선언하여 재정의를 하는 것을 말한다.
