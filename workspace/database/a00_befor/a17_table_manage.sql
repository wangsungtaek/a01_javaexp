/*
# 테이블 관리
1. 테이블의 생성과 구조의 변경(추가, 수정, 삭제)을 처리하는 것을 말한다.
2. 테이블 생성
...
3. 테이블 생성 기본 형식
	...
	옵션 2 : 스키마(사용자계정) - 다른 계정에서 호출해서 사용한다.
		현재 계정에서 사용할 테이블은 스키마 선언을 생략가능하다.
...
4. 컬럼의 변경
	...
	4) 기존 칼럼에 데이터가 있는 경우
		...
		ps) 기존 데이터가 있는 경우에 특정한 데이터 유형으로 변환이 필요한 경우
			1. key(primary key) empno의 함께 특정 임시컬럼으로 저장
			2. update 테이블명
				set 컬럼 = Null; -- 타입을 변경할 컬럼은 null로 update 처리
			3. 컬럼 타입변경.
			4. 해당 타입에 맞는 유형을 to_XXX()함수를 이용해서
				임시컬럼에 있는 데이터를 할당한다.
			5. 필요없는 임시컬럼을 삭제 처리.
